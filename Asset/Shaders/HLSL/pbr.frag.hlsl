...
float4 pbr_frag_main(pbr_vert_output input) : SV_Target
{
    ...
    // return (saturate(0.15f + (0.2f * BaseColorMap.Sample(samp0, input.TextureUV) * 0.4f) + (specLighting * 0.8f)));
    // return saturate((0.3f * diffuseLighting) + normalMap.Sample(samp0, input.TextureUV) + (specLighting * 0.5f));
    // return saturate(BaseColorMap.Sample(samp0, input.TextureUV) 
    //             + metallicRoughnessMap.Sample(samp0, input.TextureUV)
    //             + normalMap.Sample(samp0, input.TextureUV)
    //             + occlusionMap.Sample(samp0, input.TextureUV)
    //             + emissiveMap.Sample(samp0, input.TextureUV));
    return saturate(BaseColorMap.Sample(samp0, input.TextureUV) + emissiveMap.Sample(samp0, input.TextureUV));
}
