...
int main()
{
	...
	// ------------------------------------
	std::string vsPath = g_pAssetLoader->GetFilePath("Shaders/GLSL/texture.vs");
	std::string fsPath = g_pAssetLoader->GetFilePath("Shaders/GLSL/texture.fs");
	...
	int colorComponent = m_Image.bitcount / m_Image.bitdepth;
	GLint color = colorComponent == 3 ? GL_RGB : GL_RGBA;
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, color, m_Image.Width, m_Image.Height, 0, color, GL_UNSIGNED_BYTE, data);
