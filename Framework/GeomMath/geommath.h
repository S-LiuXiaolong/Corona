...
namespace Corona {
	template<typename T, uint32_t SizeOfArray>
	constexpr uint32_t countof(T(&)[SizeOfArray]) { return SizeOfArray; }

	template<typename T, uint32_t RowSize, uint32_t ColSize>
	constexpr uint32_t countof(T(&)[RowSize][ColSize]) { return RowSize * ColSize; }
